setwd("d:/Work/Dropbox (UFL)/NeuroImage/simulations/new-simus/D")
#################################################
rm(list=ls())
gc()
...
S_true <- vector('list', m)
for(j in 1:m){
    S_true[[j]] <- matrix(0, p, 2*p)
}

for(r in 1:(p-1)){
    S_true[[1]][r, r+1] <- -0.6
    S_true[[1]][r, r+p+1] <- 0.3
    S_true[[2]][r, r+1] <- 0.75
    S_true[[2]][r, r+p+1] <- -0.4
    S_true[[3]][r, r+1] <- -0.6
    S_true[[3]][r, r+p+1] <- 0.3
}

phi.full <- S_true[[1]]
for(i in 2:m){
    phi.full <- cbind(phi.full, S_true[[i]])
}  

#display the true AR coefficients matrice
print(plot.matrix((phi.full), m*q.t))
...
for(epoch in 1:max_iter){
    ...
    for(j in 2:length(final.estimated.intervals)){
        ...
        var.fit <- fitVAR(interval_data, p = 2, penalty = 'ENET', alpha = 1, intercept = FALSE)
        temp_mat <- cbind(var.fit$A[[1]], var.fit$A[[2]])
...
save(cases_ret, file = "results_D2.RData")
