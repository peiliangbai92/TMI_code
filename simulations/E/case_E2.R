...
for(mm in 1:m){
    ...
    if(mm %% 3 == 1){
        temp <- diag(c(rep(1, p/2),rep(0, p/2)))
        set.seed(10000)
        g <- erdos.renyi.game(p/2, 0.2, type = "gnp")
        temp[-(1:(p/2)),-(1:(p/2))] <- as.matrix(get.adjacency(g))
        phi.full[,((mm-1)*q.t*p+1):((mm)*q.t*p)]  <- temp
    }else if(mm %% 3 == 2){
        set.seed(123456)
        g <- erdos.renyi.game(p, 0.1, type = "gnp")
        temp <- as.matrix(get.adjacency(g))
        phi.full[,((mm-1)*q.t*p+1):((mm)*q.t*p)]  <- temp
    }else{
        temp <- diag(c(rep(0, p/2), rep(1, p/2)))
        set.seed(123321)
        g <- erdos.renyi.game(p/2, 0.2, type = "gnp")
        temp[(1:(p/2)),(1:(p/2))] <- as.matrix(get.adjacency(g))
        phi.full[,((mm-1)*q.t*p+1):((mm)*q.t*p)]  <- temp
...
for(mm in 1:m){
    ...
    max_eigen <- max(abs(eigen(temp)$values))
